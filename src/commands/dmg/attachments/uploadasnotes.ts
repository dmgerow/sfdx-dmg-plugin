...
export default class UploadAsNotes extends SfdxCommand {
  public static description = messages.getMessage("uploadasfiles.description");
  public static examples = [];

  public static readonly flagsConfig = {
    source: flags.string({
      char: "s",
      description: messages.getMessage("uploadasfiles.flags.source"),
      required: true,
    }),
    target: flags.string({
      char: "t",
      description: messages.getMessage("uploadasfiles.flags.target"),
      required: true,
    }),
  };
  ...
  public async run(): Promise<any> {
    ...
    const randomNumberString = Math.floor((Math.random() * 100000) + 1).toString();
    this.successWriter.pipe(
      fs.createWriteStream(join(target, "success_files" + randomNumberString + ".csv"), { flags: "w" })
    );
    this.errorWriter.pipe(
      fs.createWriteStream(join(target, "error_files" + randomNumberString + ".csv"), { flags: "w" })
    );
  ...
  private async uploadFile(path, attachment) {
    ...
    let fileName = join(path, attachment["Title"] + ".snote");
    ...
    let requestBody = {
      ...
      PathOnClient: attachment["Title"] + ".snote",
