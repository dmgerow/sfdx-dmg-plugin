...
const messages = Messages.loadMessages("sfdx-dmg-plugin", "workbench");

export default class WorkbenchOpen extends SfdxCommand {
  ...
  public async run(): Promise<JsonMap> {
    ...
    if (targetWorkbenchUrl) {
      ...
      if (setDefaultworkbenchUrl) {
        ...
        this.ux.log(
          `Stored ${targetWorkbenchUrl} in ${globalWorkbenchFilePath}.\n`
        );
      }
    } else {
      // Check the default
    ...
    if (urlOnly) {
      this.ux.log(
        `Access workbench as user ${org.getUsername()} with the following URL: ${workbenchUrlWithSid}`
      );
    } else {
      this.ux.log(
        `Opening ${targetWorkbenchUrl} and logging into ${org.getUsername()}.`
      );
  ...
  private getDefaultWorkbenchFilePath() {
    ...
    const pluginPath = join(globalFolder, "vendors", "sfdx-dmg-plugin");
